...
def check_credentials(username, password):
    ...
    with open('BufferData\\CurrentUser\\username.txt', 'r') as f_username, open('BufferData\\CurrentUser\\password.txt', 'r') as f_password:
...
class AccountDelete(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.label1 = customtkinter.CTkLabel(self.frame, text="Enter Your Credential",font=customtkinter.CTkFont(size=18, weight="bold"))
        self.label1.grid(row=1, column=0, padx=30, pady=(50, 15))
        
        self.username_entry = customtkinter.CTkEntry(self.frame, width=200, placeholder_text="Username")
        self.username_entry.grid(row=2, column=0, padx=30, pady=(15, 15))
         
        self.password_entry = customtkinter.CTkEntry(self.frame, width=200, show="*", placeholder_text="Password")
        self.password_entry.grid(row=3, column=0, padx=30, pady=(0, 15))
        
        self.login_button = customtkinter.CTkButton(self.frame, text="Erase Data", command=self.Erase_event, width=200)
        self.login_button.grid(row=4, column=0, padx=30, pady=(15, 2.5))

        self.login_button = customtkinter.CTkButton(self.frame, text="Feedback Button", command=self.Feedback_dialog, width=200)
