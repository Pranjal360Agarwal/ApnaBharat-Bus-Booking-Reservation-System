...
def new_run():
    ...
    Label(f17,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red').grid(row=1,column=0,columnspan=5)
    ...
    Label(f18,text = "Running Date (DD-MM-YY)", font=("Arial",15),fg='light green').grid(row=0,column=2,padx=20)
    e24 = Entry(f18)
    ...
    def add_run():
        ...
        if len(already):
            ...
            cur.execute('insert into runs(bus_id,journey_date,seat_available) values(?,?,?)',(int(e23.get()),e24.get(),e25.get()))
            con.commit()
            cur.execute('select * from runs where bus_id='+e23.get()+' and journey_date="'+e24.get()+'"')
            row=cur.fetchall()
            row=row[0]
    ...
    def delete_run():
        cur.execute('DELETE FROM runs WHERE bus_id='+e23.get()+' AND journey_date="'+e24.get()+'"')
        con.commit()

        showinfo("Deleted","Bus running details deleted successfully!!")
        
    button20 = Button(f18,text = "Add Run",font=("Arial",15),bg='light green',command=add_run).grid(row=0,column=6,padx=20,pady=20)
    button21 = Button(f18,text = "Delete",font=("Arial",15),fg='red',bg='light green',command=delete_run).grid(row=0,column=7,padx=20,pady=20)

    button22 = Button(f17,image=home_img,anchor=CENTER,command = tab2).grid(row=4,column=0,columnspan=5)
...
def new_route():
    ...
    Label(f15,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red').grid(row=1,column=0,columnspan=5)
    ...
    def add_route():
        ...
        if len(already):
            ...
            cur.execute('insert into route (route_id ,station ,s_id) values (?,?,?)',(int(e20.get()),e21.get(),int(e22.get())))
            cur.execute('select * from route where route_id='+e20.get()+' and s_id='+e22.get())
            row=cur.fetchall()
            row=row[0]
            con.commit()
            showinfo("Successful","Data added successfully\n"+str(row))
    ...
    def delete_route():
        cur.execute('DELETE FROM route WHERE route_id=? and station=? and s_id=?',(int(e20.get()),e21.get(),int(e22.get())))
        con.commit()
        showinfo("Deleted","Route Deleted Successfully")

    button20 = Button(f16,text = "Add Route",font=("Arial",15),bg='light green',command=add_route).grid(row=0,column=6,padx=20,pady=20)
    button21 = Button(f16,text = "Delete",font=("Arial",15),fg='red',bg='light green',command=delete_route).grid(row=0,column=7,padx=20,pady=20)

    button22 = Button(f15,image=home_img,anchor=CENTER,command = tab2).grid(row=4,column=0,columnspan=5)
...
def new_bus():
    ...
    Label(f13,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red').grid(row=1,column=0,columnspan=5)
    ...
    Label(f14,text = "Fare Rs ", font=("Arial",15),fg='light green').grid(row=0,column=6,padx = 15)
    e17 = Entry(f14)
    e17.grid(row=0,column=7)
    Label(f14,text = "Operator ID", font=("Arial",15),fg='light green').grid(row=0,column=8, padx = 19)
    e18 = Entry(f14)
    e18.grid(row=0,column=9)
    Label(f14,text = "Route ID", font=("Arial",15),fg='light green').grid(row=0,column=10, padx = 19)
    e19 = Entry(f14)
    ...
    def add_bus():
        ...
        if len(already):
            ...
            cur.execute('insert into bus (bus_id,type,capacity,fare,route_id,op_id) values (?,?,?,?,?,?)',(int(e14.get()),menu1.get(),int(e16.get()),int(e17.get()),int(e19.get()),int(e18.get())))
            cur.execute('select * from bus where bus_id='+e14.get()+' and route_id='+e19.get())
            row=cur.fetchall()
            row=row[0]
            con.commit()
    ...
    button17 = Button(f14,text = "Add Bus",font=("Arial",15),bg='light green',anchor=CENTER,command=add_bus).grid(row=1,column=4,padx=20,pady=20)
    button18 = Button(f14,text = "Delete",font=("Arial",15),bg='light green',anchor=CENTER,command=delete_bus).grid(row=1,column=6,padx=20,pady=20)

    button19 = Button(f13,image=home_img,anchor=CENTER,command = tab2).grid(row=4,column=0,columnspan=5)
...
def new_operator():
    ...
    Label(f11,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red').grid(row=1,column=0,columnspan=5)
    ...
    Label(f12,text = "Operator Id ", font=("Arial",12),fg='green').grid(row=0,column=0,padx = 20)
    e9 = Entry(f12)
    e9.grid(row=0,column=1)
    Label(f12,text = "Name ", font=("Arial",12),fg='green').grid(row=0,column=2,padx=20)
    e10 = Entry(f12)
    e10.grid(row=0,column=3)
    Label(f12,text = "Address ", font=("Arial",12),fg='green').grid(row=0,column=4,padx= 20)
    e11 = Entry(f12)
    e11.grid(row=0,column=5)
    Label(f12,text = "Phone ", font=("Arial",12),fg='green').grid(row=0,column=6,padx = 10+10)
    e12 = Entry(f12)
    e12.grid(row=0,column=7)
    Label(f12,text = "E-mail", font=("Arial",12),fg='green').grid(row=0,column=8, padx = 19+1)
    e13 = Entry(f12)
    ...
    def add_op():
        ...
        if len(already):
            ...
            cur.execute('select * from operator where op_id='+str(e9.get()))
            row=cur.fetchall()
            row=row[0]
            con.commit()
            showinfo("Successful","Data added successfully\n"+str(row))
    ...
    def delete_op():
        cur.execute('DELETE FROM operator WHERE op_id='+e9.get()+' and name="'+e10.get()+'" and address="'+e11.get()+'" and phone='+e12.get()+' and email="'+e13.get()+'"')
        con.commit()
        
        showinfo("Deleted","Operator Details Deleted Successfully")

    button15 = Button(f12,text = "Add",font=("Arial",15),bg='light green',command=add_op).grid(row=1,column=4,padx=20)
    button16 = Button(f12,text = "Delete",font=("Arial",15),bg='light green',command=delete_op).grid(row=1,column=6,padx=20)

    button14 = Button(f11,image=home_img,anchor=CENTER,command = tab2).grid(row=4,column=0,columnspan=5)
...
def ticketShow(row):
    ...
    Label(f4,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red').grid(row=1,column=0,columnspan=5)
    Label(f4,text = "BUS TICKET", font=("Arial",20), bg='light green', fg='green4',bd=5).grid(row=3,column=0,columnspan = 5,pady=20)
    final = LabelFrame(f4)
    ...
    Label(final,text="Gender: "+row[2]+"                      ").grid(row=6, column = 7,columnspan = 3)
    Label(final,text="Phone: "+str(row[0])+"              ").grid(row=7, column = 7,columnspan = 3)
    Label(final,text="Fare Rs: "+str(row[7])+"                  ").grid(row=8, column = 7,columnspan = 3)
    Label(final,text="Bus Id: "+str(row[4])+"                       ").grid(row=9, column = 7,columnspan = 3)
    Label(final,text="Boarding Point: "+row[8]+"           ").grid(row=10, column = 7,columnspan = 3)
    Label(final,text="Total Rs. "+str(row[7]*row[6])+" you have to pay at the time of boarding: ").grid(row=11, column = 0,columnspan = 3)
    showinfo("Successful","Ticket Book Sucessfully")
    button2 = Button(f4,image=home_img,anchor=CENTER,command = tab2).grid(row=16,column=2,pady=50)
...
def seat_book():
    ...
    def proceed_to(f5,busch,fare):
        if type(busch)!=int:
            ...
            Label(f6,text = "Fill Passenger Details To Book The Bus Ticket", font=("Arial",25), bg = 'deep sky blue', fg = 'red').grid(row=0,column=0,columnspan=15)
            ...
            def booked(fare):
                if e6.get()=="" or e4.get()=="" or e7.get()=="" or e3.get()=="" or e5.get()=="" or e2.get()=="" or e1.get()=="":
                    showerror("Invalid Data","Please fill all the data!")
                else:                    
                    insertRow = (int(e6.get()),e4.get(),menu.get(),int(e7.get()),busch,e3.get(),int(e5.get()),fare,e2.get(),e1.get())
                    fare*=insertRow[6]
                    cur.execute('select seat_available from runs where bus_id='+str(busch)+' and journey_date="'+e3.get()+'"')
                    ...
                    if int(e5.get())<seats_avl:
                        ...
                        if resp:
                            ...
                            cur.execute('update runs set seat_available='+str(seats_avl-int(e5.get()))+' where bus_id='+str(busch)+' and journey_date="'+e3.get()+'"')
                            con.commit()
                            ticketShow(insertRow)
                    else:
                        showerror("Limited Seats","Insufficient seats avilable in the bus!")

            button8 = Button(f6,text="Book Seat", font=("Arial",15), activebackground = 'light green', bg = 'SpringGreen3',command = lambda: booked(fare)).grid(row=1,column=10,padx=20,pady=20)

    def show_bus(f5):
        if e1.get()=="" or e2.get()=="" or e3.get()=="":
            showinfo("Invalid Inputs","Please enter all the details!")
        else:
            data=cur.execute('select b.bus_id,o.name,b.type,t.seat_available,b.fare from route r,route s,runs t,bus b,operator o where o.op_id=b.op_id and r.route_id=s.route_id and t.bus_id=b.bus_id and b.route_id=r.route_id and t.journey_date= "'+str(e3.get())+'" and r.station="'+str(e1.get())+'" and s.station="'+str(e2.get())+'" and r.s_id>s.s_id')
            f6=Frame(f5)
            ...
            button7 = Button(f6,text="Proceed to Book", font=("Arial",15), activebackground = 'light green', bg = 'SpringGreen3',command = lambda : proceed_to(f5,busch.get(),row[4])).grid(row=1,column=5,padx=20,pady=20)
...
def check_booked_seat(f2):
    ...
    Label(f7,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red',anchor=CENTER ).grid(row=1,column=0,columnspan=5)
    ...
    def checkHistory():
        ...
        if(len(e8.get())!=10):
            ...
            cur.execute('select name,sex,ticket,mobile,age,fare,ref_id,bus_id,fromst,tost,journey_date from booking where Mobile='+str(e8.get())+' order by ref_id desc limit 1')
            con.commit()
            details=cur.fetchall()
            i=5
            for row in details:
                box = Label(f8, text = 'Name: '+row[0]+'\nGender: '+row[1]+'\nNo. of seats: '+str(row[2])+'\nPhone: '+str(row[3])+'\nAge: '+str(row[4])+'\nFare Rs: '+str(row[5])+'\nBooking ref: '+str(row[6])+'\nBus Id: '+str(row[7])+'\nBoarding point: '+row[8]+'\nDestination: '+row[9]+'\nTravel On: '+row[10]+'\n\n *Total amount Rs '+str(row[5]*row[2])+' to be paid at time of boarding the bus',font = 'bold', relief = 'groove')
    ...
    button9 = Button(f7,image=home_img,anchor=CENTER,command = tab2).grid(row=4,column=0,columnspan=5)
...
def add_bus():
    ...
    Label(f9,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red',anchor=CENTER ).grid(row=1,column=0,columnspan=5)
    ...
    button23 = Button(f9,image=home_img,anchor=CENTER,command = tab2).grid(row=4,column=0,columnspan=5)
...
def tab2():
    ...
    Label(f2,text = "Python Bus Service", font=("Arial",25), bg = 'deep sky blue', fg = 'red',anchor=CENTER ).grid(row=1,column=0,columnspan=5)
    f3 =Frame(f2,pady=20)
    ...
    button4 = Button(f2,image=home_img,anchor=CENTER,command = tab2).grid(row=3,column=0,columnspan=5)
